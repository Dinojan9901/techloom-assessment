...
import { listRefunds, refundOrder } from '../services/refund.service.js';
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { order, refund } = await cancelOrder({ orderId: req.params.id, reason });

    res.json({
      order,
      refund,
      message: refund
        ? `Order cancelled and ${(refund.amountCents / 100).toFixed(2)} refunded to the original payment method.`
 ...
 * Reverses a captured payment and returns the units to the shelf. Cancelling a
 * paid order does the same thing; this is the explicit form.
 */
router.post(
  '/:id/refund',
  idempotent('order.refund'),
  asyncHandler(async (req, res) => {
    const { reason } = parseBody(cancelInput, req.body ?? {});
    const { order, refund } = await refundOrder({ orderId: req.params.id, note: reason ?? '' });

    res.json({
      order,
      refund,
      message: `Refunded ${(refund.amountCents / 100).toFixed(2)} to the original payment method.`,
    });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [payments, refunds] = await Promise.all([
      listPayments(order._id),
      listRefunds(order._id),
    ]);
