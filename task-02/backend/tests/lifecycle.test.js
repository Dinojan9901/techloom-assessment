...
describe('order lifecycle', () => {
  ...
  test('cancelling a paid order refunds it and puts the units back on the shelf', async () => {
    const product = await createProduct({ totalStock: 6 });
    const { response } = await checkoutOne({ productId: product._id, quantity: 2 });
    const orderId = response.body.order.id;
    ...
    const cancelled = await api().post(`/api/orders/${orderId}/cancel`).expect(200);

    // Money moved, so cancelling has to reverse the charge, not just the stock.
    expect(cancelled.body.order.status).toBe('REFUNDED');
    expect(cancelled.body.refund.amountCents).toBe(cancelled.body.order.totalCents);
