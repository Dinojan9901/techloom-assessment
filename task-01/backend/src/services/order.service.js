...
const orderNumber = () =>
  `POS-${Date.now().toString(36).toUpperCase()}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
...
/**
 * Cancels an order and puts its stock back where it belongs — releasing a hold
 * for an unpaid order, restoring the shelf for a paid one.
 */
export async function cancelOrder({ orderId, reason = 'Cancelled by user' }) {
  return withTransaction(async (session) => {
    ...
    const order = await transitionOrder({
      orderId,
      from: [...HOLDS_RESERVED_STOCK, ...HOLDS_SOLD_STOCK],
    ...
    if (HOLDS_SOLD_STOCK.includes(current.status)) {
      // The sale had settled, so the units are back on the shelf.
      await restoreStock(order.items, session);
    } else if (order.reservation) {
      await releaseReservation({ reservationId: order.reservation, reason, session });
    }
